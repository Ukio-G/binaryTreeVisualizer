...
void ImGui::TextEx(const char* text, const char* text_end, ImGuiTextFlags flags)
{
    ...
    if (text_end - text > 2000 && !wrap_enabled)
    {
        ...
        // - From this point we will only compute the width of gridLines that are visible. Optimization only available when word-wrapping is disabled.
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth(), (is_multiline ? g.FontSize * 8.0f : label_size.y) + style.FramePadding.y*2.0f); // Arbitrary default of 8 gridLines high for multi-line
    ...
    if (render_cursor || render_selection)
    {
        ...
        {
            // Find gridLines numbers straddling 'cursor' (slot 0) and 'select_start' (slot 1) positions.
            ...
            // Iterate all gridLines to find our line numbers
            // In multi-line mode, we never exit the loop until all gridLines are counted, so add one extra to the searches_remaining counter.
        ...
        if (render_selection)
        {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect_size.x <= 0.0f) rect_size.x = IM_FLOOR(g.Font->GetCharAdvance((ImWchar)' ') * 0.50f); // So we can see selected empty gridLines
...
void ImGui::EndColumns()
{
    ...
    if (!(flags & ImGuiColumnsFlags_NoBorder) && !window->SkipItems)
    {
        ...
        // Apply dragging after drawing the column gridLines, so our rendered gridLines are in sync with how items were displayed during the frame.
